...
import {
  CyclePhaseDetails,
  cyclePhaseDetailsMap,
  CyclePhaseEnum,
} from '../core/domain/cycle-phase.enum';
...
function getCycleDay(currentDate: Date, menstruationStartDate: Date) {
  const diffTime = Math.abs(
    currentDate.getTime() - menstruationStartDate.getTime()
  );
...
export class FileCycleRepository implements CycleRepository {
  getCurrentPhase(today: Date): CyclePhaseEntity {
    try {
      ...
      return this.calculateCycleData(
        new Date(data.menstruationStartDate),
        today
      );
    } catch (err) {
  ...
  private calculateCycleData(
    ...
    const daysUntilNextPhase =
      cycleDay >= 14 ? cycleLength - cycleDay + 1 : 14 - cycleDay;
